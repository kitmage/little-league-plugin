...
class LLLM_Admin {
    ...
    public static function render_welcome() {
        ...
        echo '<li><code>[lllm_schedule]</code>: <code>.lllm-shortcode-heading</code>, <code>.lllm-schedule</code>, <code>.date-time</code>, <code>.location</code>, <code>.home</code>, <code>.away</code>, <code>.win</code>, <code>.day</code>, <code>.date</code>, <code>.time</code>, <code>.lllm-team</code>, <code>.lllm-team-name</code>, <code>.lllm-team-score</code>, <code>.lllm-updated</code>.</li>';
    ...
    public static function render_seasons() {
        ...
        if (!$seasons) {
            ...
            foreach ($seasons as $season) {
                ...
                echo '<input type="hidden" name="lllm_delete_confirmed" value="0">';
    ...
    public static function render_games() {
        ...
        $team_options = array();
        foreach ($team_rows as $team_row) {
            $team_code = (string) $team_row->team_code;
            if (!isset($team_map[$team_code])) {
                continue;
            }
        ...
        echo '<details style="margin:0 0 16px;">';
        echo '<summary style="cursor:pointer;font-weight:600;">' . esc_html__('Add Game Manually', 'lllm') . '</summary>';
        if (!$team_options) {
            echo '<p>' . esc_html__('Assign teams to this division before creating games manually.', 'lllm') . '</p>';
        } else {
            echo '<form class="lllm-manual-game-form" method="post" action="' . esc_url(admin_url('admin-post.php')) . '" style="margin-top:12px;display:grid;grid-template-columns:repeat(2,minmax(180px,1fr));gap:10px;max-width:900px;">';
            wp_nonce_field('lllm_create_game_manual');
            echo '<input type="hidden" name="action" value="lllm_create_game_manual">';
            echo '<input type="hidden" name="season_id" value="' . esc_attr($season_id) . '">';
            echo '<input type="hidden" name="division_id" value="' . esc_attr($division_id) . '">';
            echo '<label>' . esc_html(sprintf(__('Date/Time (%s)', 'lllm'), $site_timezone_label)) . '<br><input type="date" name="start_date" required></label>';
            echo '<label>' . esc_html__('Start Time', 'lllm') . '<br><input type="time" name="start_time" step="300" required></label>';
            echo '<label>' . esc_html__('Location', 'lllm') . '<br><input type="text" name="location" class="regular-text" required></label>';
            echo '<label>' . esc_html__('Status', 'lllm') . '<br><select name="status" class="lllm-manual-status">';
            foreach (array('scheduled', 'played', 'canceled', 'postponed') as $status) {
                echo '<option value="' . esc_attr($status) . '">' . esc_html($status) . '</option>';
            }
            echo '</select></label>';
            echo '<label>' . esc_html__('Type', 'lllm') . '<br><select name="competition_type" required>';
            foreach (array('regular', 'playoff') as $competition_type) {
                echo '<option value="' . esc_attr($competition_type) . '">' . esc_html($competition_type) . '</option>';
            }
            echo '</select></label>';
            echo '<label>' . esc_html__('Away Team', 'lllm') . '<br><select name="away_team_code" class="lllm-team-select" required><option value="">' . esc_html__('Select away team', 'lllm') . '</option>';
            foreach ($team_options as $team_code => $franchise_name) {
                echo '<option value="' . esc_attr($team_code) . '">' . esc_html($franchise_name . ' (' . $team_code . ')') . '</option>';
            }
            echo '</select></label>';
            echo '<label>' . esc_html__('Home Team', 'lllm') . '<br><select name="home_team_code" class="lllm-team-select" required><option value="">' . esc_html__('Select home team', 'lllm') . '</option>';
            foreach ($team_options as $team_code => $franchise_name) {
                echo '<option value="' . esc_attr($team_code) . '">' . esc_html($franchise_name . ' (' . $team_code . ')') . '</option>';
            }
            echo '</select></label>';
            echo '<label class="lllm-manual-score-wrap">' . esc_html__('Away Score', 'lllm') . '<br><input type="number" name="away_score" min="0" step="1" class="small-text"></label>';
            echo '<label class="lllm-manual-score-wrap">' . esc_html__('Home Score', 'lllm') . '<br><input type="number" name="home_score" min="0" step="1" class="small-text"></label>';
            echo '<label style="grid-column:1 / -1;">' . esc_html__('Notes (optional)', 'lllm') . '<br><input type="text" name="notes" class="regular-text" style="width:100%;"></label>';
            echo '<div style="grid-column:1 / -1;display:flex;align-items:center;gap:10px;">';
            echo '<button class="button button-primary lllm-manual-submit" type="submit">' . esc_html__('Create Game', 'lllm') . '</button>';
            echo '<span class="lllm-manual-form-error" style="display:none;color:#b32d2e;">' . esc_html__('Home and away teams must be different.', 'lllm') . '</span>';
            echo '</div>';
            echo '</form>';
        }
        echo '</details>';
     ...
     * @return void
     */
    public static function handle_create_game_manual() {
        if (!current_user_can('lllm_manage_games')) {
            wp_die(esc_html__('You do not have permission to access this page.', 'lllm'));
        }

        check_admin_referer('lllm_create_game_manual');
        ...
        $home_score_raw = isset($_POST['home_score']) ? trim((string) wp_unslash($_POST['home_score'])) : '';
        $notes = isset($_POST['notes']) ? sanitize_text_field(wp_unslash($_POST['notes'])) : '';
        $competition_type = isset($_POST['competition_type']) ? self::normalize_playoff_meta_value(wp_unslash($_POST['competition_type'])) : '';

        if (!$season_id || !$division_id || $start_date === '' || $start_time === '' || $location === '' || $away_team_code === '' || $home_team_code === '' || $competition_type === '') {
            self::redirect_with_notice($return_url, 'error', __('All required fields must be provided.', 'lllm'));
        }
        ...
        if ($status === 'played') {
            if ($home_score === null || $away_score === null) {
                self::redirect_with_notice($return_url, 'error', __('Scores are required for played games.', 'lllm'));
            }
        } else {
            $away_score = null;
            $home_score = null;
        ...
        if (!in_array($competition_type, array('regular', 'playoff'), true)) {
            self::redirect_with_notice($return_url, 'error', __('Invalid competition type.', 'lllm'));
        }
    ...
    private static function render_delete_confirmation_script() {
        ...
        echo '<script>';
        echo '(function(){';
        echo 'var forms=document.querySelectorAll(".lllm-delete-confirm-form, .lllm-bulk-delete-form");';
        echo 'forms.forEach(function(form){form.addEventListener("submit",function(event){var submitter=event.submitter||null;var message=(submitter&&submitter.getAttribute("data-confirm-message"))||form.getAttribute("data-confirm-message")||"Are you sure?";if(!window.confirm(message)){event.preventDefault();return;}var confirmedField=form.querySelector("input[name=\\"lllm_delete_confirmed\\"]");if(confirmedField){confirmedField.value="1";}});});';
        echo '})();';
        echo '</script>';
    }

    /**
     * Checks whether a delete request has browser-confirmation flag set.
     *
     * @return bool True when browser confirmation is present.
     */
    private static function is_browser_delete_confirmed() {
        if (!isset($_POST['lllm_delete_confirmed'])) {
            return false;
        }

        return sanitize_text_field(wp_unslash($_POST['lllm_delete_confirmed'])) === '1';
    }
    ...
    public static function handle_delete_season() {
        ...
        if (!$season_id || !self::is_browser_delete_confirmed()) {
    ...
    public static function handle_bulk_delete_seasons() {
        ...
        if (!$season_ids || !self::is_browser_delete_confirmed()) {
            self::redirect_with_notice(admin_url('admin.php?page=lllm-seasons'), 'delete_blocked', __('Confirmation required.', 'lllm'));
        }
    ...
    public static function handle_delete_division() {
        ...
        $season_id = isset($_POST['season_id']) ? absint($_POST['season_id']) : 0;
        if (!$division_id || !self::is_browser_delete_confirmed()) {
    ...
    public static function handle_bulk_delete_divisions() {
        ...
        if (!$division_ids || !self::is_browser_delete_confirmed()) {
            self::redirect_with_notice(admin_url('admin.php?page=lllm-divisions&season_id=' . $season_id), 'delete_blocked', __('Confirmation required.', 'lllm'));
        }
    ...
    public static function handle_delete_team() {
        ...
        if (!$team_id || !self::is_browser_delete_confirmed()) {
    ...
    public static function handle_bulk_delete_teams() {
        ...
        if (!$team_ids || !self::is_browser_delete_confirmed()) {
            self::redirect_with_notice(admin_url('admin.php?page=lllm-franchises'), 'delete_blocked', __('Confirmation required.', 'lllm'));
        }
    ...
    public static function handle_delete_game() {
        ...
        $season_id = isset($_POST['season_id']) ? absint($_POST['season_id']) : 0;
        $division_id = isset($_POST['division_id']) ? absint($_POST['division_id']) : 0;
        if (!$game_id || !self::is_browser_delete_confirmed()) {
    ...
    public static function handle_bulk_delete_games() {
        ...
        if (!$game_ids || !self::is_browser_delete_confirmed()) {
            self::redirect_with_notice(admin_url('admin.php?page=lllm-games&season_id=' . $season_id . '&division_id=' . $division_id), 'delete_blocked', __('Confirmation required.', 'lllm'));
        }
